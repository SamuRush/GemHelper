...
- **Адаптивный детектор `IWakeWordDetector` (`Voice/WakeWord/`)**:
  ...
  - **Защита KWS от ложных срабатываний (Token Matching)**: проверка детекции вейк-ворда выполняется строго через скомпилированный `Regex` по границам изолированных слов `\b{customName}\b`. Подстрочный поиск (`Contains`) полностью исключён, что предотвращает ложные срабатывания на фонетические обрывки («джа», «рис», «вис»), дыхание и фоновый шум.
...
**Защита от самоперехвата речи (Acoustic Echo Suppression):**  
Во время воспроизведения ответа ассистента микрофон `VoiceListener` аппаратным образом глушится (флаги `_isPaused` и `_isSpeaking`). Входящие PCM-кадры микрофона полностью отбрасываются до попадания в KWS и STT, буферы сбрасываются мгновенно, а после окончания речи выдерживается обязательное защитное окно затухания акустического эха колонок в **300 мс** в блоке `finally` перед повторной активацией микрофона.
