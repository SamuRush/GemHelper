...
- [`Voice/WakeWord/VoskGrammarWakeWordDetector.cs`](file:///c:/Users/evsee/OneDrive/Desktop/Gem/Voice/WakeWord/VoskGrammarWakeWordDetector.cs):
  ...
  - **Защита KWS от ложных срабатываний (Token Matching)**: проверка детекции вейк-ворда выполняется строго через скомпилированный `Regex` по границам изолированных слов `\b{customName}\b` (`_wakeWordRegex`). Подстрочные проверки (`Contains`) запрещены, что полностью блокирует ложные срабатывания на фонетические обрывки («джа», «рис», «вис») и дыхательный/фоновый шум.
...
- [`Services/VoiceListener.cs`](file:///c:/Users/evsee/OneDrive/Desktop/Gem/Services/VoiceListener.cs):
  ...
  - **Acoustic Echo Suppression (глушение микрофона на время TTS)**: двухконтурная защита от самоперехвата собственного голоса ассистента из динамиков. Во время синтеза выставляются флаги `_isPaused` и `_isSpeaking`, входящие PCM-фреймы микрофона на аппаратном уровне отбрасываются до попадания в KWS и STT, буферы распознавателя мгновенно сбрасываются через `Reset()`, а разблокировка микрофона выполняется строго после защитного окна затухания эха (300 мс) в блоке `finally`.
...
- [`Services/CompositeVoiceFeedbackService.cs`](file:///c:/Users/evsee/OneDrive/Desktop/Gem/Services/CompositeVoiceFeedbackService.cs):
  ...
  - Координация с Vosk: пауза микрофона перед речью, Acoustic Echo Suppression (блокировка аудио-фреймов флагом `_isSpeaking`), 300 мс кулдаун в блоке `finally` для гарантированного затухания акустического эха колонок, активация `EnterConfirmationListening` при наличии активного `PendingAction`.
